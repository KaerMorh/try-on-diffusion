...
class AFWM(nn.Module):
    ...
    def __init__(self, opt, input_nc):
        ...
        self.image_features = FeatureEncoder(3 + 18, num_filters)
    ...
    def forward(self, cond_input, image_input, kpt):
        cond_pyramids = self.cond_FPN(self.cond_features(cond_input))  # maybe use nn.Sequential
        image_pyramids = self.image_FPN(self.image_features(torch.cat([image_input, kpt], dim=1)))
