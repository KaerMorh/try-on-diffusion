...
def main():
    ...
    dataset = DFPairDataset('/data/user/gjh/DeepFashion', opt.H, arbitrary_mask_percent=0.1, state='test')
    # dataset = CPDataset('/data/user/gjh/VITON-HD', opt.H, mode='test', unpaired=opt.unpaired)
    ...
    coarse_path = os.path.join(outpath, "initial")
    sample_path = os.path.join(outpath, "source")
    result_path = os.path.join(outpath, "unpaired")
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                for data in iterator:
                    ...
                    # last_image = Image.open(os.path.join(outpath, "results", filename[:-4] + '.png'))
                    # last_image = transforms.Resize((512, 512), interpolation=transforms.InterpolationMode.BILINEAR)(last_image)
                    # last_image_tensor = transforms.ToTensor()(last_image).unsqueeze(0)
                    # cloth_image_tensor = torch.clamp((cloth_image_tensor + 1.0) / 2.0, min=0.0, max=1.0)
                    # x_result = last_image_tensor * (1 - cloth_mask_tensor) + cloth_image_tensor * cloth_mask_tensor
                    ...
                    ################# two step inference ################
                    # cloth_mask_tensor = cloth_mask_tensor.to(device)
                    # cloth_image_tensor = cloth_image_tensor.to(device)
                    # for i in range(1):
                    #     x_first_result = x_sample_result.cpu() * (1 - mask_tensor) + image_tensor * mask_tensor
                    #     new_start = x_first_result.to(device) * (1 - cloth_mask_tensor) + cloth_image_tensor * cloth_mask_tensor
                    #
                    #     # save_x = resize(new_start[0])
                    #     # save_x = torch.clamp((save_x + 1.0) / 2.0, min=0.0, max=1.0)
                    #     # save_x = 255. * rearrange(save_x.cpu().numpy(), 'c h w -> h w c')
                    #     # img = Image.fromarray(save_x.astype(np.uint8))
                    #     # img.save(os.path.join(result_path, filename[:-4] + "_{}.png".format(i*2+1)))
                    #
                    #     z_start = model.encode_first_stage(new_start)
                    #     z_start = model.get_first_stage_encoding(z_start).detach()
                    #     ts = torch.full((1,), 999, device=device, dtype=torch.long)
                    #     start_code = model.q_sample(z_start, ts)
                    #
                    #     shape = [opt.C, opt.H // opt.f, opt.W // opt.f]
                    #     samples_ddim, _ = sampler.sample(S=opt.ddim_steps,
                    #                                      conditioning=c,
                    ...
                    if not opt.skip_save:
                        ...
                        for i, x_sample in enumerate(x_result):
                            ...
                            # save_gt = resize(image_tensor[i])
                            # save_gt = 255. * rearrange(un_norm(save_gt).cpu(), 'c h w -> h w c').numpy()
                            # GT_img = Image.fromarray(save_gt.astype(np.uint8))
                            # GT_img.save(os.path.join(sample_path, filename[:-4] + ".png"))
                            #
                            # save_feat = resize(feat_tensor[i])
                            # save_feat = 255. * rearrange(un_norm(save_feat).cpu(), 'c h w -> h w c').numpy()
                            # init_img = Image.fromarray(save_feat.astype(np.uint8))
                            # init_img.save(os.path.join(coarse_path, filename[:-4] + ".png"))
