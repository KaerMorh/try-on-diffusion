...
def main():
    ...
    dataset = DFPairDataset('/home/bingxing2/gpuuser392/gjh/Datasets/DeepFashion', 512, arbitrary_mask_percent=0.1, state='test')
    ...
    examples = []
    if ',' in str(opt.example_num):
        nums = opt.example_num.split(',')
        for num in nums:
            examples.append(int(num))
    else:
        examples.append(int(opt.example_num))
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                for example in examples:
                    data = dataset[example]
                    # data['ref_imgs'] = dataset[opt.example_num+100]['ref_imgs']
                    filename = '{}_example_{}.jpg'.format(version, example)
                    if opt.use_init_feat:
                        filename = 'n{}_'.format(opt.noise_step) + filename
                    if opt.use_ilvr:
                        filename = 'ilvr{}_'.format(opt.ilvr_factor) + filename
                    ...
                    test_model_kwargs['feat'] = feat_tensor.to(device)
                    ...
                    warp_feat = model.encode_first_stage(test_model_kwargs['feat'])
                    warp_feat = model.get_first_stage_encoding(warp_feat).detach()
                    ts = torch.full((1,), opt.noise_step, device=device, dtype=torch.long)
                    ...
                    init_warp = model.decode_first_stage(warp_feat)
                    def un_norm(x):
                        return (x + 1.0) / 2.0
                    ...
                    if not opt.skip_save:
                        for i, x_sample in enumerate(x_checked_image_torch):
                            ...
                            all_img.append(un_norm(init_warp[i].cpu()))
