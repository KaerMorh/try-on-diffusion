...
from ldm.data.try_on import mask2bbox
...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # aligned_c, aligned_cm = pre_alignment(c_img, cm_img, mask)
        # aligned_c = self.transform(aligned_c)
        # aligned_cm_array = np.array(aligned_cm)
        # aligned_cm_array = (aligned_cm_array >= 128).astype(np.float32)
        # aligned_cm = torch.from_numpy(aligned_cm_array)  # [0,1]
        # aligned_cm.unsqueeze_(0)
        # warped_cloth = aligned_c * aligned_cm
        ...
        # inpaint = im * inpaint_mask

        warped_cloth_name = im_name.replace('image', 'cloth-warp' if not self.unpaired else 'unpaired-cloth-warp')
        if self.fine_width > 512:
            warped_cloth_name = warped_cloth_name.replace('cloth-warp', 'cloth-1024-warp')
        ...
        if self.fine_width > 512:
            warped_cloth_mask_name = warped_cloth_mask_name.replace('cloth-warp', 'cloth-1024-warp')
        ...
        # inpaint_img = (inpaint + 1.0) / 2.0
        # feat_img = (feat + 1.0) / 2.0
        # transforms.ToPILImage()(inpaint_img).save('inpaint_img.jpg')
        # transforms.ToPILImage()(feat_img).save('feat_img.jpg')
        result = {
            "GT": im,
